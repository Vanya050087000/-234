--[[
    FATALITY.WIN UI FRAMEWORK - FULL VERSION
    Rainbow Border + ESP + Watermark
]]
...
function Fatality:SetTheme(accent, secondary)
    ...
    for _, grad in pairs(self.Gradients) do
        if grad and grad.Parent then grad.Color = newGradient end
    end
...
function Fatality:AddWatermark()
    ...
    Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = Frame})
    ...
    Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = TopInfo})
...
function Fatality:EnableESP(settings)
    ...
    local function createESP(player)
        ...
        if settings.Tracers then
            ...
            local conn = RunService.RenderStepped:Connect(updateTracer)
...
function Fatality:CreateWindow(titleText)
    local Window = {Tabs = {}, Visible = true, CurrentTab = nil}
    ...
    Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = Loading})
    ...
    Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = SearchContainer})
        ...
        for _, tab in pairs(Window.Tabs) do
            for _, section in pairs(tab.Sections) do
                ...
                for _, element in pairs(section.Elements) do
                    local match = element.Name:lower():find(query)
    ...
    function Window:AddTab(name)
        ...
        table.insert(self.AccentObjects, Page)
        ...
        function Tab:AddSection(sName)
            ...
            Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = SectionFrame})
            ...
            function Elements:AddToggle(text, default, flag, callback)
                ...
                local Box = Create("TextButton", {
                    ...
                    BackgroundColor3 = Tgl.Value and self.Accent or Theme.Outline,
                    Text = "",
                    BorderSizePixel = 0,
                    Parent = Frame
                })
                ...
                function Tgl:Set(val)
                    Tgl.Value = val
                    Box.BackgroundColor3 = val and self.Accent or Theme.Outline
                    callback(val)
                end
                ...
                if flag then self.Options[flag] = Tgl end
            ...
            function Elements:AddSlider(text, min, max, default, flag, callback)
                ...
                if flag then self.Options[flag] = Sld end
                table.insert(Section.Elements, {Name = text, Frame = Frame})
                return Sld
            end
            ...
            function Elements:AddColorPicker(text, default, flag, callback)
                ...
                Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = Preview})
                ...
                Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = PickerFrame})
                ...
                Preview.MouseButton1Click:Connect(function() 
                    ...
                    if PickerFrame.Visible then updateInternal() end
                end)

                if flag then self.Options[flag] = CP end
                table.insert(Section.Elements, {Name = text, Frame = Frame})
                return CP
            end
            ...
            function Elements:AddKeybind(text, default, flag, callback)
                ...
                Btn.MouseButton1Click:Connect(function() 
                    picking = true; Btn.Text = "..." 
                end)
                UserInputService.InputBegan:Connect(function(i)
                    if picking and i.UserInputType == Enum.UserInputType.Keyboard then
                        picking = false; Bind:Set(i.KeyCode.Name)
                    end
                end)

                if flag then self.Options[flag] = Bind end
                table.insert(Section.Elements, {Name = text, Frame = Frame})
                return Bind
            end
            ...
            function Elements:AddButton(text, callback)
                ...
                Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = Btn})
            ...
            function Elements:AddDropdown(text, list, default, flag, callback)
                ...
                Create("UIStroke", {Color = Theme.Outline, Thickness = 2, Parent = DropFrame})
                Create("UIListLayout", {Parent = DropFrame})
                ...
                for _, val in pairs(list) do
                    ...
                    Item.MouseButton1Click:Connect(function()
                        Btn.Text = val; Dd.Value = val; toggle(false); callback(val)
                    end)
                end

                Btn.MouseButton1Click:Connect(function() toggle(not DropFrame.Visible) end)
                function Dd:Set(val) Btn.Text = val; Dd.Value = val; callback(val) end
                if flag then self.Options[flag] = Dd end
                table.insert(Section.Elements, {Name = text, Frame = Frame})
                return Dd
            end
...
function Fatality:Notify(title, text, duration)
    ...
    Create("TextLabel", {
        ...
        TextColor3 = self.Accent, Font = self.Font, TextSize = 14,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = Frame
    })

    Create("TextLabel", {
        Size = UDim2.new(1, -20, 0, 25), Position = UDim2.new(0, 10, 0, 28),
        BackgroundTransparency = 1, Text = text,
        TextColor3 = Theme.Text, Font = self.Font, TextSize = 12,
        TextXAlignment = Enum.TextXAlignment.Left, Parent = Frame
    })
